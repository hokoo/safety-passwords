...
PR #21 tracks `release/1.4.3-ready` against `master`; the branch name is retained while the candidate version changes to 1.5. The previous product and test behavior passed at `257234d`; this version metadata change requires its own scoped review. Automatic WordPress.org release preparation is a separate pending task, not implemented by this version edit. The deferred E2 branch remains outside the PR.
